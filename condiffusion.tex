...
\tikzstyle{process2}=[draw, font=\fontsize{5}{4.8}, rectangle, rounded corners, fill=cyan!20, text width=2cm, auto, on grid, align=center]
...
\node[process3] (seq2seq) at (-3, -0.35) {2.Diff Seq2Seq};
...
\node[process2] (prior) at (1, -0.35) {1.Conditioned DiffPrior};
...
\node[process2] (optimization) at (1, -2.4) {3.Latent Optimization};
\draw[vecArrow] (prior) -- (optimization);
\draw[vecArrow] (1.7, -1.95) tonode[anchor=east] {\scriptsize $\nabla \mathcal{L}$} (1.7, -0.75);
